"""
Job Description Generator V2 - Anthropic/Claude Version
...
This version uses Anthropic's Claude models.
...
scope_agent = Agent(
    ...
    CRITICAL: You must return ALL FOUR fields as strings. Never omit a field or use null/None.
...
async def generate_job_description(user_responses: UserResponses, org_context: OrganizationalContext) -> JobDescription:
    ...
    """
    print(f"\n{'='*80}")
    print(f"🚀 STARTING JOB DESCRIPTION GENERATION (Anthropic)")
    print(f"{'='*80}")
    ...
    try:
        ...
        print(f"   ❌ AGENT 1 FAILED: {type(e).__name__}: {str(e)}")
        raise
    ...
    try:
        ...
        print(f"   ✓ Generated {len(key_responsibilities.responsibilities)} responsibilities")
        print(f"   ✓ Tokens: {usage.request_tokens} in, {usage.response_tokens} out")
    except Exception as e:
        print(f"   ❌ AGENT 2 FAILED: {type(e).__name__}: {str(e)}")
        raise
    ...
    try:
        ...
        print(f"   ✓ Type: {people_management.type_of_role}")
        print(f"   ✓ Tokens: {usage.request_tokens} in, {usage.response_tokens} out")
    except Exception as e:
        print(f"   ❌ AGENT 3 FAILED: {type(e).__name__}: {str(e)}")
        raise
    ...
    Generate all four Scope subsections (REQUIRED - do not omit any):
    1. Contacts (Typical) - must be a non-empty string
    2. Innovation - must be a non-empty string
    3. Decision Making - must be a non-empty string
    4. Impact of Results - must be a non-empty string
    ...
    try:
        ...
        print(f"   ✓ Scope sections generated successfully")
        print(f"   ✓ Tokens: {usage.request_tokens} in, {usage.response_tokens} out")
    except Exception as e:
        print(f"   ❌ AGENT 4 FAILED: {type(e).__name__}: {str(e)}")
        raise
    ...
    try:
        ...
        print(f"   ✓ Found {len(licenses_certs.requirements)} requirements")
        print(f"   ✓ Tokens: {usage.request_tokens} in, {usage.response_tokens} out")
    except Exception as e:
        print(f"   ❌ AGENT 5 FAILED: {type(e).__name__}: {str(e)}")
        raise
    ...
    try:
        ...
        print(f"   ✓ Working conditions generated successfully")
        print(f"   ✓ Tokens: {usage.request_tokens} in, {usage.response_tokens} out")
    except Exception as e:
        print(f"   ❌ AGENT 6 FAILED: {type(e).__name__}: {str(e)}")
        raise
    ...
    cost_usd = (
        (total_input_tokens / 1_000_000) * config.ANTHROPIC_SONNET_INPUT_COST +
        (total_output_tokens / 1_000_000) * config.ANTHROPIC_SONNET_OUTPUT_COST
    )
    ...
    print(f"💵 Total Cost: ${cost_usd:.4f} USD (${cost_cad:.4f} CAD)")
    print(f"{'='*80}\n")
    print("🔧 Assembling final job description...")
    ...
    print("✅ Job description assembly complete!\n")
